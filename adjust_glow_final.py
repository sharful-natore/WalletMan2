...
def replacer(match):
    ...
    # glowSize = 4.dp (user request)
    # glowLayers = 30 (more blur/smoother)
    # alpha = 0.025f (softer)
    return f"""{indent}// Android 8.1 compatible natural outward glow
{indent}val glowLayers = 30
{indent}val glowSize = 4.dp.toPx()
...
{indent}    drawArc(
{indent}        color = {color_var}.copy(alpha = 0.025f),
...
pattern = r"([ \t]*)// Android 8.1 compatible natural outward glow\s*val glowLayers = 20[\s\S]*?color = (.*?)\.copy\(alpha = 0\.035f\),[\s\S]*?startAngle = (.*?),[\s\S]*?sweepAngle = (.*?),[\s\S]*?style = androidx\.compose\.ui\.graphics\.drawscope\.Stroke\(width = currentGlowWidth, cap = androidx\.compose\.ui\.graphics\.StrokeCap\.Butt\)[\s\S]*?\n[ \t]*\}"
