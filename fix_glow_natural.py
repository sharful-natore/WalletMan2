...
def replacer(match):
    ...
    # 15 layers for much smoother gradient (no rings)
    # alpha starts low and decreases further
    return f"""{indent}// Android 8.1 compatible natural outward glow
{indent}val glowLayers = 15
{indent}val glowSize = 12.dp.toPx()
...
{indent}    drawArc(
{indent}        color = {color_var}.copy(alpha = 0.04f),
...
# 3. startAngle
# 4. sweepAngle
pattern = r"([ \t]*)// Android 8.1 compatible outward glow\s*val glowLayers = 5[\s\S]*?color = (.*?)\.copy\(alpha = 0\.15f\),[\s\S]*?startAngle = (.*?),[\s\S]*?sweepAngle = (.*?),[\s\S]*?style = androidx\.compose\.ui\.graphics\.drawscope\.Stroke\(width = currentGlowWidth, cap = androidx\.compose\.ui\.graphics\.StrokeCap\.Butt\)[\s\S]*?\n[ \t]*\}"
